...
seeds_checkpoint = torch.load(checkpoint_path_seeds, map_location=device)['net_dict']
seeds_model.load_state_dict(seeds_checkpoint)
seeds_model.to(device)
seeds_model.eval()

ostia_model = OstiapointsNet()
ostia_checkpoint = torch.load(checkpoint_path_ostia, map_location=device)['net_dict']

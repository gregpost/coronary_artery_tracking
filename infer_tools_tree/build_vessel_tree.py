...
def infer(start: list):
    ...
    if not (
        ...
        debug.SaveNumpyArrayToFile(new_patch, 'new_patch', image)
...
def search_line(start, curr_r, direction, prob_records, root: TreeNode):
    ...
    if find_node_forward is not None:
        ...
        # SavePointListAsMPS([point[::-1] for point in point_list], f'data/seeds{point_counter}.mps', image)
        # point_counter += 1
...
# seed_point = [166, 160, 190]
# root = TreeNode([182, 187, 187], start_point_index=None)
#
# direction, prob_records, curr_r = search_first_node(seed_point, [0] * 3)
# search_line(start=seed_point, curr_r=curr_r, prob_records=prob_records, direction=direction, root=root)

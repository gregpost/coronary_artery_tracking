...
class Trainer(object):
    def __init__(self, batch_size, num_workers, train_dataset, val_dataset, model, model_name, optimizer, criterion,max_points=500, save_num = 0,start_epoch=0, max_epoch=1000, initial_lr=0.01, checkpoint_path=None, best_test_loss=2**31):
    ...
    def train_step(self, epoch):
        ...
        print_str = "Train Loss:{:.5f} Direction Train Loss:{:.5f} Radius Train Loss:{:.5f}".format(
    ...
    def val_step(self, epoch):
        ...
        if True:
            for idx, (inputs, labels, r) in enumerate(self.train_loader):
                ...
                test_loss += loss.item()
                total += labels.size(0)
        print_str = "Val Loss:{:.5f} Direction Val Loss:{:.5f} Radius Val Loss:{:.5f}".format(test_loss / len(self.train_loader),
                                                                                                                        val_direction_loss / len(self.train_loader),
                                                                                                                        val_radius_loss / len(self.train_loader))

        self.print_to_log_file(print_str)
        print("test loss",test_loss/len(self.train_loader))
        print("best test loss", self.best_test_loss)
        if (test_loss/len(self.train_loader)) < self.best_test_loss:
            print("saving models")
            self.best_test_loss = test_loss/len(self.train_loader)
            save_fold = "../checkpoint/classification_checkpoints"
            if not os.path.exists(save_fold):
                os.makedirs(save_fold)
            model_save_path = save_fold+"/"+ self.model_name + "_model_s"+str(self.save_num)+".pkl"
        ...
        return test_loss / len(self.train_loader), 1. - correct / total, val_direction_loss/ len(self.train_loader), val_radius_loss/ len(self.train_loader)
    ...
    def run_train(self):
        ...
        for epoch in range(self.start_epoch, self.max_epoch):
            print(f'centerline_trainner run_train epoch: {epoch}')

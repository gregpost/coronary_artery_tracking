...
def get_dataset(save_num=0):
    ...
    train_pre_fix_path = "../data_process_tools/patch_data/ostia_patch"
    train_flag = 'train'
    ...
    val_pre_fix_path = "../data_process_tools/patch_data/ostia_patch"
    val_flag = 'val'
...
if __name__ == '__main__':
    data_path = os.path.join('..', 'data_process_tools', 'patch_data', 'ostia_patch')
    ...
    if start_epoch != 0:
        ...
        ostia_checkpoint = torch.load(model_checkpoint_path)
        model.load_state_dict(ostia_checkpoint['net_dict'])

        inital_lr = ostia_checkpoint['initial_lr']
        curr_loss = ostia_checkpoint['curr_loss']

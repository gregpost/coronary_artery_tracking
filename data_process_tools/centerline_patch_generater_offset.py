            ...
            if j % gap_size == 0:
                ...
                if org_x_pixel != last_center_x_pixel or org_y_pixel != last_center_y_pixel or org_z_pixel != last_center_z_pixel:
                    ...
                    for k in range(10):
                        ...
                        while True:
                            if (
                                    center_x_pixel != org_x_pixel or center_y_pixel != org_y_pixel or center_z_pixel != org_z_pixel) and (
                            center_x_pixel, center_y_pixel, center_z_pixel) not in record_set:
                        ...
                        if pre_ind != -1 and next_ind != -1:
                                                ...
                                                if 0 <= ind_x < re_spacing_img.shape[1]:
                                                    for ind_y in range(left_y, right_y + 1):
                                                        if 0 <= ind_y < re_spacing_img.shape[2]:
                                                            ...
                                                            new_src_arr[ind_z - left_z] = src_temp[ind_x, ind_y]
                                    break
                            ...
                            folder_path = './patch_data/centerline_patch/offset/point_' + str(
                                max_points) + '_gp_' + str(gap_size) + '/' + 'd' + str(i)
                            if not os.path.exists(folder_path):
                                os.makedirs(folder_path)
                            record_name = 'centerline_patch/offset/point_' + str(max_points) + '_gp_' + str(
                                gap_size) + '/' + 'd' + str(i) + '/' + 'd_' + str(i) + '_' + 'v_' + str(
                                v) + '_' + 'patch_%d_' % counter + str(k) + '.nii.gz'
...
def create_patch_images(max_points,path_name,spacing_path,gap_size):
    ...
    for dataset in datasets:
        ...
        if error_count > 0:
            ...
            info_file_path: str = "./patch_data/centerline_patch/offset/" + 'point_' + str(max_points) + '_gp_' + str(gap_size) + '/' + 'd' + str(ds_idx) + '.txt'

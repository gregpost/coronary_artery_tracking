...
def create_ds(data_path):
    offset_dir = os.path.join(data_path, 'offset')
    no_offset_dir = os.path.join(data_path, 'no_offset')
...
def move_images(df, image_dir, dest_dir):
    ...
    for index, row in df.iterrows():
        print(f'centerline index: {index}')
...
path_data = os.path.abspath(os.path.join('patch_data', 'centerline_patch'))

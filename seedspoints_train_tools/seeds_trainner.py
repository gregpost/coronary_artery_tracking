...
class Trainer(object):
    def __init__(self, batch_size, num_workers, train_dataset, val_dataset, model, model_name, optimizer, criterion, save_num = 0,start_epoch=0, max_epoch=1000, initial_lr=0.01, checkpoint_path=None, best_test_loss=2**31):
    ...
    def val_step(self, epoch):
        ...
        if True:
            for idx, (inputs, labels) in enumerate(self.train_loader):
                ...
                test_loss += loss.item()
                total += labels.size(0)
        print_str = "Val Loss:{:.5f} Total:{:}".format(test_loss / len(self.train_loader),total)
    ...
    def run_train(self):
        ...
        for epoch in range(self.start_epoch, self.max_epoch):
            print(f'seeds_trainner run_train epoch: {epoch}')
